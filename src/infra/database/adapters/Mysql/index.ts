...
import { QAttributes, Options } from '../../@types'
...
class MysqlAdapter implements Database {
  ...
  connect() {
    ...
    return new Promise((resolve, reject) => {
      self.connection = Mysql.createConnection(this.options)
  ...
  query(attributes:QAttributes):Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.connection.query(query, [], (err, res) => {
        ...
        if (res.affectedRows && this.attributes.command === 'insert') {
          return resolve(res.insertId)
        }

        if (res.affectedRows && this.attributes.command === 'update') {
        ...
        return resolve(this.keysToLowerCase(res))
      })
    })
  }
